...
class FormulirController extends Controller
{
    ...
    public function index()
    {
        ...
        $formTexts = Storage::get('forms.txt');

        $exploded = collect(preg_split("/\r\n|\n|\r/", $formTexts))->filter(fn ($text) => Str::of($text)->contains('(') && Str::of($text)->contains(')'));

        $labels = $exploded->map(function ($label) {
            return [
                'key' => Str::of($label)->betweenFirst('(', ')')->__toString(),
                'text' => Str::of($label)->after(')')->__toString()
            ];
        })->keyBy('key');

        $inputs = collect($forms)->flatMap(function ($form, $path) use ($labels) {
            return collect($form)->map(function ($input, $key) use ($path, $labels) {
                return [
                    'path' => $path,
                    'type' => collect(explode('\\', get_class($input)))
                        ->last(),
                    'key' => $key,
                    'input' => $input
                ];
    ...
    public function store(Request $request)
    {
        ...
        function draw(
            ...
            $isDebug = false;
            // $isDebug = true;
            if ($fillable instanceof Text) {
                $image->text(
                    $fillable->text,
                    $fillable->x,
                    $fillable->y,
                    function ($font) use ($fontStyle, $fontColor, $fillable) {
                        $font->file($fontStyle);
                        $font->size($fillable->fontSize);
                        $font->color($fontColor);
                        $font->kerning($fillable->kerning);
                        $font->align($fillable->align);
                        $font->valign('top');
                    }
                );
            } elseif ($fillable instanceof Box) {
                $image->rectangle(
                    $fillable->x,
                    $fillable->y,
                    $fillable->x + $fillable->width,
                    $fillable->y + $fillable->height,
                    function ($draw) {
                        $draw->border(2, 'rgba(220, 38, 38, 0.5)');
                    }
                );
            } elseif ($fillable instanceof TextInput) {
                $left = $fillable->x;
                $y = $fillable->y;

                for ($i = 0; $i < $fillable->length; $i++) {
                    $isOdd = $i % 2 === 0;

                    $offset = $isOdd ? 2 : 1;

                    $x1 = $left;
                    $x2 = $x1 + $fillable->width;

                    //draw the box with offset of each other and make sure they are not overlap
                    if ($isDebug) {
                        $image->rectangle(
                            $x1,
                            $y,
                            $x2,
                            $y + $fillable->height,
                            function ($draw) use ($isOdd) {
                                $draw->border(1, $isOdd ? 'rgba(220, 38, 38, 0.5)' : 'rgba(38, 220, 38, 0.5)');
                            }
                        );
                    }

                    // draw the text
                    if ($i < strlen($fillable->text)) {
                        $image->text(
                            strtoupper($fillable->text[$i]),
                            $x1 + $fillable->width / 2,
                            $y + $fillable->height / 2,
                            function ($font) use ($fontStyle, $fontColor, $fillable) {
                                $font->file($fontStyle);
                    ...
                    if ($i % 25 === 0 && $i !== 0) {
                        // Reset x position when it reach the end of the line
                        $left = $fillable->x + $offset - 1;
                        $y = $fillable->y + $fillable->height + 2;
                    } else {
                        $left = $x2 + $offset + 1;
                    }
                }
            } else if ($fillable instanceof DateInput) {
                $date = $fillable->date;
                ...
                $dayInput = new TextInput(
                    x: $fillable->dayOffset,
                    y: $fillable->y,
                    width: $fillable->width,
                    height: $fillable->height,
                    length: 2,
                    text: $day,
                );

                $monthInput = new TextInput(
                    x: $fillable->monthOffset,
                    y: $fillable->y,
                    width: $fillable->width,
                    height: $fillable->height,
                    length: 2,
                ...
                draw($image, $fontStyle, $fontColor, $dayInput);
                draw($image, $fontStyle, $fontColor, $monthInput);
                draw($image, $fontStyle, $fontColor, $yearInput);
            } else if ($fillable instanceof DayInput) {
                $date = $fillable->date;
                ...
                $dayOfWeekInput = new TextInput(
                    x: $fillable->weekdayOffset,
                    y: $fillable->y,
                    width: $fillable->width,
                    height: $fillable->height,
                    length: 2,
                    text: str_pad($weekday, 2, '0', STR_PAD_LEFT),
                );
                ...
                draw($image, $fontStyle, $fontColor, $dayOfWeekInput);
                draw($image, $fontStyle, $fontColor, $dayInput);
                draw($image, $fontStyle, $fontColor, $monthInput);
                draw($image, $fontStyle, $fontColor, $yearInput);
            } else if ($fillable instanceof SelectInput) {
                $selectedOption = $fillable->selectedOption;

                for ($i = 0; $i < count($fillable->options); $i++) {
                    $option = $fillable->options[$i];

                    $x1 = $option['x'] ?? $fillable->x;
                    $x2 = $x1 + $fillable->width;

                    $y1 = $option['y'] ?? $fillable->y;
                    $y2 = $y1 + $fillable->height;
                    ...
                    if ($isDebug) {
                        $image->rectangle(
                            ...
                            $x2,
                            $y2,
                            function ($draw) use ($option, $selectedOption) {
                                $draw->border(1, $option['id'] === $selectedOption ? 'rgba(220, 38, 38, 0.5)' : 'rgba(38, 220, 38, 0.5)');
                            }
                        );
                    }
                    ...
                    if ($i === $selectedOption) {
                        $image->text(
                            $fillable->symbol,
                            $x1 + $fillable->width / 2,
                            $y1 + $fillable->height / 2,
                            function ($font) use ($fontStyle, $fontColor) {
                                $font->file($fontStyle);
                                $font->size(13);
                                $font->color($fontColor);
                                $font->align('center');
            ...
            } else if ($fillable instanceof ClockInput) {
                $time = $fillable->time;

                $hour = $time->format('H');
                $minute = $time->format('i');

                $hourInput = new TextInput(
                    x: $fillable->hourOffset,
                    y: $fillable->y,
                    width: 20,
                    height: 16,
                    length: 2,
                ...
                $minuteInput = new TextInput(
                    x: $fillable->minuteOffset,
                    y: $fillable->y,
                    width: 20,
                    height: 16,
                    length: 2,
                ...
                draw($image, $fontStyle, $fontColor, $hourInput);
                draw($image, $fontStyle, $fontColor, $minuteInput);
            }
        }
        ...
        foreach ($forms as $form => $fillables) {
            $image = Image::make($form);

            // Define font style and color
            // $fontStyle = Storage::path('Pangolin-Regular.ttf');
            $fontStyle = Storage::path('Arial.ttf');
            $fontColor = '#000000';
            ...
            foreach ($fillables as $key => $fillable) {
                ...
                if ($fillable instanceof TextInput) {
                    $fillable->text = $value;
                } else if ($fillable instanceof SelectInput) {
                    $fillable->selectedOption = (int) $value;
                } else if ($fillable instanceof DateInput) {
                    $fillable->date = Carbon::parse($value);
                } else if ($fillable instanceof DayInput) {
                    $fillable->date = Carbon::parse($value);
                } else if ($fillable instanceof ClockInput) {
                    $fillable->time = Carbon::parse($value);
                }
                draw($image, $fontStyle, $fontColor, $fillable);
            }

            // Save output image
            $path = 'public/' . $timestamp . '-' . basename($form);
            $fileList[] = $path;
            $image->save(Storage::path($path));
        }

        $zipPath = 'public/' . $timestamp . '.zip';

        $zip = new ZipArchive();
        if ($zip->open(Storage::path($zipPath), ZipArchive::CREATE | ZipArchive::OVERWRITE) === true) {
            // Loop through each file in the list and add it to the zip archive
